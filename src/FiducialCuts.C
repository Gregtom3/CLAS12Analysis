...
bool FiducialCuts::DC_fiducial_cut_theta_phi(float cx, float cy, float cz, int sector, int region, int part_pid){
  ...
  double calc_phi_min = minparams[pid][sector - 1][region][0] + minparams[pid][sector - 1][region][1] * std::log(theta_DCr) 
    + minparams[pid][sector - 1][region][2] * theta_DCr + minparams[pid][sector - 1][region][3] * theta_DCr * theta_DCr;

  double calc_phi_max = maxparams[pid][sector - 1][region][0] + maxparams[pid][sector - 1][region][1] * std::log(theta_DCr)
    + maxparams[pid][sector - 1][region][2] * theta_DCr + maxparams[pid][sector - 1][region][3] * theta_DCr * theta_DCr;
...
// Copied from EventTree.cxx in c-dilks/dispin
Bool_t FiducialCuts::CheckSampFrac(float p) {
  ...
  if(Ele_sector>=1 && Ele_sector<=6) {
    ...
    else if( _runNumber>=6120 && _runNumber<=6604 ) { // RGB spring 2019
    ...
    else if( (_runNumber>=11323 && _runNumber<=11571) || (_runNumber>=11093 && _runNumber<=11300) ) { // RGB winter 2020 // (also using this for RGB fall 2019, but it should be updated! TODO)
